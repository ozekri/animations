...
ax1.scatter(X[:, 0], X[:, 1], np.zeros_like(X[:, 0]), c=y, cmap=plt.cm.coolwarm)

# Définir les limites des axes pour une meilleure visualisation
ax1.set_xlim([-2, 2])
ax1.set_ylim([-2, 2])
ax1.set_zlim([-2, 2])  # Pour inclure la variation sur l'axe z
ax1.view_init(azim=-60, elev=20)
ax1.set_xticks([])
ax1.set_yticks([])
ax1.set_zticks([])
...
def update(frame):
    ...
    if frame >= 85:
        ...
        ax.plot_surface(xx, yy, zz, color='#fff4fc',label="Hyperplane")
